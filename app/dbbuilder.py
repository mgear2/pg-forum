...
class DBbuilder:
    ...
    def build(self, args, database):
        self._createschemasql = "CREATE SCHEMA {0}".format(database)
    ...
    def destroy(self, database):
        self._destroysql = "DROP SCHEMA {0} CASCADE".format(database)
    ...
    def count(self, table):
        ...
        count = (count[0])[0]
        print(
            "Initialization for table {0} complete with {1} rows.".format(table, count)
        )
    ...
    def buildcomments(self):
        ...
        commentstring = (
            "INSERT INTO Comments (" "comment_id, score, creation_date, text) VALUES "
        )
        tuples = ()
        i = 0
    ...
    def buildcommented(self):
        ...
        for type_tag in self.commentsroot.findall("row"):
            ...
            if int(comment_id) > self.lims[2]:
                break
            if (
                int(user_id) not in self.user_ids
    ...
    def buildthread(self):
        ...
        for type_tag in self.commentsroot.findall("row"):
            ...
            if int(comment_id) > self.lims[2]:
                break
            if (
                int(comment_id) not in self.comment_ids
    ...
    def builddecorated(self):
        ...
        decoratedstring = (
            "INSERT INTO Decorated (" "user_id, badge_id, date_awarded) VALUES "
        )
        tuples = ()
        i = 0
        ...
        for type_tag in self.badgesroot.findall("row"):
            ...
            tuples += (user_id, badge_id, datetime.strptime(date_awarded, self.datefmt))
            i += 1
